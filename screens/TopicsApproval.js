...
import Card from '../components/ApprovalCard';
...
const TopicsApproval = ({ navigation }) => {
  ...
  const getFeed = async () => {
    ...
    setRefresh(true)
    try {
      const response = await fetch(utils.ENDPONT + 'bet/pending_topics/' + userData.user_id, requestOptions);
